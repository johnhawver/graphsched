...
I think running systems should be efficient, and Kubernetes' default scheduler bugged me because it places pods that constantly talk to each other wherever they happen to fit, even though the cluster already knows the communication pattern. Every cross-node hop between two chatty pods is latency nobody had to pay for. GraphSched is my attempt to fix that — it reconstructs the dependency graph from signals already in the cluster (Service selectors, environment-variable references) and co-locates pods that depend on each other, with no changes to the workloads themselves. I picked the placement problem because I'm interested in low-latency systems, and locality is the cleanest lever there is: the less distance between two things that talk, the less you pay to move data between them.

In my test workloads it co-locates every dependent pod pair, where the default scheduler manages about 17–25%. That comes with some added scheduling latency, which I break down below.
